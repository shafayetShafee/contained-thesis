...
run_simulations <- function(
  ...
  if (sim_type == "two_lvl_int") {
    ...
    relative_bias <- as.numeric(
      ((out_mat_means["mor_hat"] - true_mor) / true_mor) * 100
    )
    ...
    relative_bias_1 <- as.numeric(
      ((out_mat_means["mor1_hat"] - true_mor1) / true_mor1) * 100
    )

    true_mor2 <- out_mat_means["true_mor2"]
    relative_bias_2 <- as.numeric(
      ((out_mat_means["mor2_hat"] - true_mor2) / true_mor2) * 100
    )
