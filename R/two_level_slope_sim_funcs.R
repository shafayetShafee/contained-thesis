...
calc_two_lvl_slp_mor_stats <- function(
  sigma_mat,
  sigma_u_sq_hat,
  var_sigma_u_sq_hat,
  x1_val
  ...
  mor_hat <- exp(
    sqrt(
      2 *
        sigma_u_sq_hat[1] +
        2 * x1_val^2 * sigma_u_sq_hat[3] +
        4 * x1_val * sigma_u_sq_hat[2]
    ) *
      qnorm(0.75)
  )
  ...
  true_mor <- exp(
    sqrt(
      2 * sigma_u1_sq + 2 * x1_val^2 * sigma_u2_sq + 4 * x1_val * sigma_u12_sq
    ) *
      qnorm(0.75)
  )
...
est_two_lvl_slope_mor <- function(m, n, fixed_coeff, sigma_mat, data_seed) {
  # data generation ----------------
  multi_data_slope <- gen_two_level_slope_data(
    m,
    n,
    fixed_coeff,
    sigma_mat,
  ...
  mor_stats_q1 <- calc_two_lvl_slp_mor_stats(
    sigma_mat,
    sigma_u_sq_hat,
    var_sigma_u_sq_hat,
    x1_q1
  ...
  mor_stats_q2 <- calc_two_lvl_slp_mor_stats(
    sigma_mat,
    sigma_u_sq_hat,
    var_sigma_u_sq_hat,
    x1_q2
  ...
  mor_stats_q3 <- calc_two_lvl_slp_mor_stats(
    sigma_mat,
    sigma_u_sq_hat,
    var_sigma_u_sq_hat,
    x1_q3
  ...
  mean_mor_hat <- mean(
    c(mor_stats_q1[2], mor_stats_q2[2], mor_stats_q3[2]),
    na.rm = FALSE
  )

  mean_se_mor_hat <- mean(
    c(mor_stats_q1[3], mor_stats_q2[3], mor_stats_q3[3]),
    na.rm = FALSE
  )
...
simulate_two_lvl_slope <- function(
  m,
  n,
  fixed_coeff,
  sigma_mat,
